...
# # В очереди анализа хранятся уже подготовленные PIL-кадры.
# # Ограничиваем размер, чтобы при 20 FPS не накопить слишком много изображений в памяти.
# PREPARED_ANALYSIS_QUEUE_MAXSIZE = 8
...
#         # preprocess_queue хранит легкие task с путями к файлам.
#         # analysis_queue хранит уже подготовленные task с PIL-изображениями.
#         self._preprocess_queue = queue.Queue()
#         self._analysis_queue = queue.Queue(maxsize=PREPARED_ANALYSIS_QUEUE_MAXSIZE)
...
#         self._preprocessed_tasks = 0
#         self._last_preprocess_time_ms = 0.0
...
#             "preprocess_queue_size": 0,
#             "preprocess_queue_frames": 0,
...
#     def _get_frames_count_in_queue(self, target_queue: queue.Queue) -> int:
#         try:
#             with target_queue.mutex:
#                 return sum(
#                     len(task.get("frame_paths", []) or [])
#                     for task in list(target_queue.queue)
...
#         preprocess_frames = self._get_frames_count_in_queue(self._preprocess_queue)
#         analysis_frames = self._get_frames_count_in_queue(self._analysis_queue)
...
#             self._total_preprocess_time_ms / self._preprocessed_tasks
#             if self._preprocessed_tasks > 0
#             else 0.0
#         )

#         return {
#             "preprocess_queue_size": self._preprocess_queue.qsize(),
#             "preprocess_queue_frames": preprocess_frames,
#             "analysis_queue_size": self._analysis_queue.qsize(),
#             "analysis_queue_frames": analysis_frames,
#             "queue_size": preprocess_frames + analysis_frames + buffered_frames,
...
#             "last_preprocess_time_ms": round(self._last_preprocess_time_ms, 2),
#             "avg_preprocess_time_ms": round(avg_preprocess_time_ms, 2),
#         }
...
#     def _record_preprocess_metric(self, preprocess_time_ms: float):
#         with self._lock:
#             self._preprocessed_tasks += 1
#             self._last_preprocess_time_ms = float(preprocess_time_ms or 0.0)
#             self._total_preprocess_time_ms += float(preprocess_time_ms or 0.0)
#             self._status.update(self._build_analysis_metrics_locked())
...
#                 self._preprocess_thread is not None
#                 and self._preprocess_thread.is_alive()
#             )
...
#                 "mode": mode,
#                 "threshold": float(threshold),
#                 "message": "Смена запущена. Кадры проходят подготовку и анализ в отдельных потоках.",
#                 "shift_id": shift_id,
...
#             "analysis_debug": bool(analysis_debug),
#             "analysis_frame_events_enabled": bool(analysis_debug),
#             "message": "Смена запущена. Камера, подготовка кадров и анализ работают раздельно.",
#         })
...
#                 "Остановка смены: новые кадры не принимаются, "
#                 "очереди подготовки и анализа дорабатываются"
#             )
...
#             "message": "Остановка смены: очереди подготовки и анализа будут обработаны до конца",
#         })
...
#                     self._status["message"] = (
#                         "Камера остановлена. Накопленные кадры переданы в очередь подготовки."
#                     )
#             return
...
#                 "message": (
#                     f"Кадр {frame_info.get('frame_name')} добавлен в буфер "
...
#         self._preprocess_queue.put(task)
...
#             "message": (
#                 f"{system_ingot_id} добавлен в очередь подготовки "
...
#     def _preprocess_worker(self, shift_id: int):
#         try:
#             while True:
#                 try:
#                     task = self._preprocess_queue.get(timeout=0.25)
#                 except queue.Empty:
#                     with self._lock:
#                         should_finish = self._stop_requested and self._preprocess_queue.empty()
...
#                 try:
#                     prepared_task = self._prepare_analysis_task(task)
#                     self._analysis_queue.put(prepared_task)
#                 finally:
#                     self._preprocess_queue.task_done()

#             # Sentinel: сообщает analysis worker, что новых подготовленных task больше не будет.
#             self._analysis_queue.put(None)

#         except Exception as e:
#             try:
#                 self._analysis_queue.put({"__error__": str(e), "shift_id": shift_id})
#             except Exception:
#                 pass

#     def _prepare_analysis_task(self, task: dict) -> dict:
#         preprocess_start = time.perf_counter()
#         prepared_frames = []

#         for frame_path in task.get("frame_paths", []) or []:
...
#         self._record_preprocess_metric(preprocess_time_ms)
...
#                 try:
#                     if isinstance(task, dict) and task.get("__error__"):
#                         raise RuntimeError(task["__error__"])
...
#                 shift_id=shift_id,
#                 status="stopped",
#                 message="Смена остановлена пользователем. Очереди подготовки и анализа обработаны.",
#             )
...
#         prepared_frames = task.get("prepared_frames") or None
...
#     prepared_frames: list[dict] | None = None,
# ):
#     frames_count = 0
#     max_p_crack = 0.0
#     best_frame_src = None
...
#         prepared_frames = [
#             {"frame_path": frame_path, "pil_image": None}
#             for frame_path in ingot_files
#         ]

#     def get_frame_image(frame_item: dict):
#         nonlocal image_load_time_ms

#         frame_path = frame_item.get("frame_path")
#         pil_img = frame_item.get("pil_image")
...
#         elif getattr(pil_img, "mode", None) != "RGB":
...
#         for frame_index, frame_item in enumerate(prepared_frames, start=1):
#             img_path = frame_item.get("frame_path")
#             if not img_path:
#                 continue

#             pil_img = get_frame_image(frame_item)
